...
def run(self):
    ...
    while queue:
        # ✅ kiểm tra giới hạn step và thời gian
        if self.check_limit_condition(step_count, time.perf_counter() - start_time):
            break

        cost, (playerX, playerY, boxes), steps = heappop(queue)
        state_count += 1
        self.observe(playerX, playerY, boxes, steps, costH=cost)
        ...
        for dx, dy in self.directions:
            ...
            priority = self.h(new_boxes)

            if new_state not in visited or priority < visited[new_state]:
                visited[new_state] = priority
                heappush(queue, (priority, new_state, steps + [(newX, newY)]))
                step_count += 1
